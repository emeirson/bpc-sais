<div class="bg-white dark:bg-gray-800 relative overflow-hidden">
    <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 py-4">
        ...
        <div
            ...
            <x-button.create href="{{ route('year-levels.create') }}" name="Add year level" />
    ...
    <div class="overflow-x-auto border">
        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
            <x-table.thead :headers="['#', 'level', 'order']" />
            <tbody>

                @forelse ($levels as $level)
                    <tr class="border-b dark:border-gray-700" wire:key="{{ $level->id }}">
                        <x-table.data>
                            {{ $loop->iteration }}
                        </x-table.data>
                        <x-table.data>
                        ...
                        <td class="px-4 py-3 flex items-center justify-end gap-x-2">
                            <button id="dropdown-button-{{ $loop->iteration }}"
                                ...
                                wire:key="dropdown-button-{{ $level->id }}" data-dropdown-placement="top"
                            ...
                            <div id="dropdown-{{ $loop->iteration }}" wire:key="dropdown-menu-{{ $level->id }}"
                                class="hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
                                <ul class="py-1 text-sm text-gray-700 dark:text-gray-200"
                                    aria-labelledby="dropdown-button-{{ $loop->iteration }}">
                                    <li>
                                        <a href="{{ route('year-levels.show', $level->id) }}"
                                            class="block py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">Show</a>
                                    </li>
                                    <li>
                                        <a href="{{ route('year-levels.edit', $level->id) }}"
                                            class="block py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">Edit</a>
                                    </li>
                                </ul>
                                <div class="py-1">
                                    <form id="delete-form-{{ $loop->index }}"
                                        action="{{ route('year-levels.destroy', $level->id) }}" method="post">
                                        @csrf
                                        @method('Delete')
                                    </form>
                                    <x-button.delete
        ...
        <x-table.pagination>{{ $levels->links() }} </x-table.pagination>
    </div>
</div>
