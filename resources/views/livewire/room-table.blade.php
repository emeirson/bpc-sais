<div class="bg-white dark:bg-gray-800 relative overflow-hidden">
    <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 py-4">
        ...
        <div
            ...
            <x-button.create href="{{ route('rooms.create') }}" name="Add room" />
    ...
    <div class="overflow-x-auto border">
        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
            <x-table.thead :headers="['#', 'code', 'name', 'capacity']" />
            <tbody>

                @forelse ($rooms as $room)
                    <tr class="border-b dark:border-gray-700" wire:key="room-{{ $room->id }}">
                        ...
                        <td class="px-4 py-3 flex items-center justify-end gap-x-2">
                            <button id="dropdown-button-{{ $loop->iteration }}"
                                ...
                                wire:key="dropdown-button-{{ $room->id }}" data-dropdown-placement="top"
                            ...
                            <div id="dropdown-{{ $loop->iteration }}" wire:key="dropdown-menu-{{ $room->id }}"
                                class="hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
        ...
        <x-table.pagination>{{ $rooms->links() }} </x-table.pagination>
    </div>
</div>
