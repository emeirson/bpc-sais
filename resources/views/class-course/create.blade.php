<x-layout.app>
    ...
    <section class="bg-white dark:bg-gray-900 pb-20">
        <div class="px-8 max-w-6xl">
            <h2 class="mb-4 text-xl font-bold text-gray-900 dark:text-white">Add a new class</h2>
            <form action="{{ route('class-course.store') }}" method="post">
                @csrf
                <div class="grid gap-4 sm:grid-cols-4 sm:gap-6">
                    <div class="sm:col-span-1">
                        <x-form.label for="class_code" value="Class Code" />
                        <x-form.input type='text' id="class_code" name="class_code" value="{{ old('class_code') }}"
                    ...
                    <div class="sm:col-span-1">
                        ...
                        <select name="semester_id" id="semester_id"
                            ...
                            @foreach ($terms as $term)
                                <option @selected(old('semester_id') == $term->id) value="{{ $term->id }}">
                                    {{ $term->details() }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="sm:col-span-1">
                        ...
                        <select name="course_id" id="course_id"
                            ...
                            @foreach ($courses as $course)
                                <option @selected(old('course_id') == $course->id) value="{{ $course->id }}">
                    ...
                    <div class="sm:col-span-1">
                        ...
                        <select name="instructor_id" id="instructor_id"
                            ...
                            @foreach ($instructors as $instructor)
                                <option @selected(old('instructor_id') == $instructor->id) value="{{ $instructor->id }}">
                    ...
                    <div class="sm:col-span-1">
                        ...
                        <select name="room_id" id="room_id"
                            ...
                            @foreach ($rooms as $room)
                                <option @selected(old('room_id') == $room->id) value="{{ $room->id }}">
                ...
                <x-button.submit value="Add class" />
            </form>
        </div>
    </section>
</x-layout.app>
