<x-layout.app>
    ...
    <section class="bg-white dark:bg-gray-900 pb-20">
        <div class="px-8 max-w-6xl">
            <h2 class="mb-4 text-xl font-bold text-gray-900 dark:text-white">Editing: {{ $level->name }}</h2>
            <form action="{{ route('year-levels.update', $level->id) }}" method="post">
                @csrf
                @method('PATCH')
                <div class="grid gap-4 sm:grid-cols-4 sm:gap-6">
                    ...
                    <div class="sm:col-span-1">
                        ...
                        <x-form.input type='number' id="order" name="order"
                            value="{{ old('order', $level->order) }}" placeholder="Enter level order" />
                        <x-form.error for="order" />
                    </div>
                </div>
                <x-button.submit value="Update year level" />
            </form>
        </div>
    </section>
</x-layout.app>
