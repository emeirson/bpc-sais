<x-layout.app>
    ...
    <section class="bg-white dark:bg-gray-900 pb-20">
        <div class="px-8 max-w-6xl">
            <h2 class="mb-4 text-xl font-bold text-gray-900 dark:text-white">Edit: {{ $section->section_code }}</h2>
                ...
                <div class="grid gap-4 sm:grid-cols-4 sm:gap-6">
                    <div class="sm:col-span-4">
                        ...
                        <select name="semester_id" id="semester_id"
                            ...
                            @forelse ($semesters as $semester)
                                <option @selected(old('semester_id', $section->semester_id) == $semester->id) value="{{ $semester->id }}">
                                    {{ $semester->details() }}
                                </option>
                            @empty
                    ...
                    <div class="sm:col-span-1">
                        ...
                        <x-form.input type='text' id="section_code" name="section_code"
                            value="{{ old('section_code', $section->section_code) }}" placeholder="Enter section code" />
                    ...
                    <div class="sm:col-span-1">
                        ...
                        <x-form.input type='number' min="1" id="year_level" name="year_level"
                            value="{{ old('year_level', $section->year_level) }}" placeholder="Enter year level" />
                        <x-form.error for="year_level" />
                    </div>
                    <div class="sm:col-span-1">
                        <x-form.label for="section" value="Section" />
                        <x-form.input type='number' min="1" id="section" name="section"
                            value="{{ old('section', $section->section) }}" placeholder="Enter section number" />
                        <x-form.error for="section" />
                    </div>
                </div>
                <x-button.submit value="Add section" />
            </form>
        </div>
    </section>
</x-layout.app>
